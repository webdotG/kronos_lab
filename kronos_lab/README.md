# kronos_lab - переезд и запуск

чистая структура исследовательского стенда, все данные и настройки в одном месте
...
папку kronos_lab положить в корень проекта рядом с kronos_model:
~/aProject/trading/
├── kronos_model/      <- библиотека модели (уже есть)
└── kronos_lab/        <- наш стенд

config.py сам найдёт kronos_model по относительному пути, ничего прописывать не надо
